 ...
 * Used by both the dev relay server and the desktop app (in-process, via IPC).
 */
...
export interface RelayOptions {
  ...
  verbose?: boolean
  /** Which runtimes to watch. Defaults to AGENTICVISUALIZER_RUNTIME env var, or 'auto'. */
...
function resolveRuntimeMode(explicit?: RelayRuntimeMode): RelayRuntimeMode {
  ...
  const raw = process.env.AGENTICVISUALIZER_RUNTIME
...
export async function createRelay(options: RelayOptions): Promise<Relay> {
  ...
  return {
    ...
    dispose() {
      // Defense in depth — callers or hot-reload could call this twice.
